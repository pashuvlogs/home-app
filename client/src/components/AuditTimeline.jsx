...
const actionConfig = {
  create: { color: 'bg-cyan-500', icon: Plus, label: 'Created' },
  save: { color: 'bg-slate-500', icon: Save, label: 'Saved' },
  submit: { color: 'bg-blue-500', icon: Send, label: 'Submitted' },
  approve: { color: 'bg-emerald-500', icon: CheckCircle, label: 'Approved' },
  reject: { color: 'bg-red-500', icon: XCircle, label: 'Rejected' },
  defer: { color: 'bg-orange-500', icon: Pause, label: 'Deferred' },
  defer_complete: { color: 'bg-orange-400', icon: CheckCircle, label: 'Deferral Completed' },
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="text-sm rounded-lg px-2 py-1 input-glass"
          >
        ...
        <button onClick={exportCSV} className="text-sm text-cyan-400 hover:text-cyan-300 transition-colors">
          Export CSV
        </button>
      </div>
      ...
      {logs.length === 0 ? (
        ...
        <div className="space-y-0">
          {logs.map((log, i) => {
            const config = actionConfig[log.action] || { color: 'bg-slate-500', icon: Clock, label: log.action };
            const Icon = config.icon;
            return (
              <div key={log.id} className="flex gap-3">
                <div className="flex flex-col items-center">
                  <div className={`w-8 h-8 rounded-full ${config.color} flex items-center justify-center shadow-lg`}>
                    <Icon size={14} className="text-white" />
                  </div>
                  {i < logs.length - 1 && <div className="w-0.5 h-full bg-white/10 my-1" />}
                </div>
                <div className="pb-4 flex-1">
                  ...
                  {log.details?.notes && (
                    <p className="text-sm text-slate-500 mt-1 italic">"{log.details.notes}"</p>
                  )}
